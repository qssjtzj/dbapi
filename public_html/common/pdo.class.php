...
//        $this->dbh->query('SET NAMES '.self::DB_CHARSET);
    ...
    public function setConfig($hostName="", $dbUser="", $dbPwd="",$dbName="", $port="", $times=1){
        ...
        $this->dsn = 'mysql:host='.$this->hostname.';port='.$this->hostport.';dbname='.$this->dbname;
    ...
    public function getAllTable(){
    	...
    	$tables = array();
			foreach ($rows as $row){
				$items = array();
				$table = $row['Tables_in_'.$this->dbname];//self::DB_NAME];
